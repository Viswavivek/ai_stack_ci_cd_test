...
class TestSentimentQuality:
    ...
    def test_sentiment_accuracy(self, text, expected):
        ...
        assert result == expected, (
            f"Expected '{expected}' for '{text[:40]}...' " f"but got '{result}'"
        )
...
class TestSummarizationQuality:
    ...
    def test_summary_under_word_limit(self):
        ...
        assert (
            word_count <= 25
        ), f"Summary too long: {word_count} words (limit 20)"  # allow 25% tolerance
...
class TestRAGQuality:
    ...
    def test_rag_answers_correctly(self):
        ...
        assert (
            "langchain" in answer.lower() or "llm" in answer.lower()
    ...
    def test_rag_answers_faiss_question(self):
        ...
        assert (
            "meta" in answer.lower()
            or "vector" in answer.lower()
...
class TestQualityGate:
    ...
    def test_sentiment_gate(self):
        ...
        correct = sum(
            1 for text, expected in cases if classify_sentiment(text) == expected
        )
    ...
    def test_answer_quality_gate(self):
        ...
        assert (
            "new delhi" in answer.lower() or "delhi" in answer.lower()
